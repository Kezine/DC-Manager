...
// BASE ORTHONORMÉE d'une face (normale + haut) : fixe le ROULIS du connecteur 3D, que la rotation minimale
// +Z→n laissait libre (ports tournés de 90° dès que la normale suivait ±X). Module PUR.
const { FaceFrame } = D("geometry/FaceFrame.js");
...
const { FacePanelBands } = D("geometry/FacePanelBands.js");   // bandes boîtier/oreilles d'une face de panneau 19″ (éditeur de façade ⇄ aperçu de la fiche)
...
const { WebglHostVisibility } = D("core/WebglHostVisibility.js");   // « l'hôte du moteur 3D WebGL doit-il être visible ? » — règle UNIQUE consommée par DcBase.render (pur)
const { OptionSearch } = D("core/OptionSearch.js");   // sélecteur d'entité À RECHERCHE : filtrage des options + règles de valeur (pur ; le DOM, lui, vit dans ui/EntityPicker)
// ÉLIGIBILITÉ d'un waypoint dans une route + rattachement erreur → étape : pur, la grammaire
// (`store/CableRouteAnalyzer`) lui est INJECTÉE. Le DOM, lui, vit dans views/forms/RouteChainEditor.
const { RouteEligibility } = D("core/RouteEligibility.js");
const { PortCompatibility } = D("core/PortCompatibility.js");   // verdict PUR d appariement de deux ports (T3)
const { ModalStack } = D("core/ModalStack.js");   // PILE de modales : push/pop + règles D9 (une seule édition vivante, arrêt de la fermeture totale) — pur ; le DOM, lui, vit dans ui/Modal
const { GlobalSearch } = D("core/GlobalSearch.js");   // recherche GLOBALE : classement par famille + plafond annoncé (pur ; la palette, elle, vit dans views/)
const { GlobalSearchSources } = D("views/GlobalSearchSources.js");   // corpus de la palette (habillage par famille + termes du module partagé SearchTerms)
const { DetailForms } = D("views/forms/DetailForms.js");   // pour DETAIL_COLLECTIONS (carte des fiches ouvrables — invariant du corpus)
...
const { CableSpline, FILLET_RADIUS_PER_K_MM } = D("geometry/CableSpline.js");   // + rayon max des congés par unité de k (style « cordes arrondies »)
...
// Bascule « Images de façade » sur les boîtes 6 faces : module PUR lui aussi (les jeux de matériaux
// sont des GÉNÉRIQUES opaques, jamais ouverts) → le choix du jeu actif et la matrice du repère
// d'orientation sont testables en Node, alors que le swap qu'ils pilotent ne l'est pas.
const { FaceImagePolicy } = D("views/dc/three/FaceImagePolicy.js");
...
const { FocusArrowMarker } = D("views/dc/three/FocusArrowMarker.js");   // style + trace PURS de la fleche de localisation
