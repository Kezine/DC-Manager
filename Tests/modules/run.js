...
const DOMAINS = [
  ...
  "./test-auth-providers.js",    // AUTHENTIFICATION (lot 3 auth/ACL) : orchestrateur `Auth` + les TROIS providers sortis de son ancien `switch` — DevAuthProvider (identité factice, jamais mise en cache), BasicAuthProvider (analyse de BASIC_AUTH = inférence de mode, décodage de l'en-tête, NON-FUITE par court-circuit prouvée en comptant les hachages), LegacySsoAuthProvider (jeton cookie nommé/complet/échappé, PASSTHROUGH intégral du JSON, replis fail-closed, jeton jamais journalisé), et l'orchestrateur lui-même (sélection du provider, annonce de boot, checkBasic du gate de transport, cache par hash de jeton réservé au sso, capture d'annuaire jamais non-loggée) — sans Express ni réseau (vue minimale de requête + `fetch` injecté)
  "./test-oidc.js",              // MODE OIDC (lot 5 auth/ACL) : l'application est elle-même le RP d'un OP (Authorization Code + PKCE) — OidcConfig (six variables en source unique, `openid` forcé dans les scopes, OIDC_COOKIE_SECURE au défaut sûr même sur coquille), GroupList (règle UNIQUE de nettoyage des groupes, invariant forward ⇄ oidc), CookieHeader (HttpOnly/Secure/SameSite=Lax, symétrie POSE ⇄ EFFACEMENT), OidcSessionStore (TTL, échéance de l'OP BORNÉE, purge, plafond, 500 ids distincts, horloge injectée), OidcAuthProvider (cookie → SsoResult, `sub` en id canonique, absent/inconnu/expiré → anonyme, ni sessionKey ni adminRight), OidcRoutes (login portant state/nonce/défi PKCE mais JAMAIS le verifier, 🚨 state faux refusé AVANT tout appel sortant, callback → cookie posé + transaction consommée, logout → session détruite et cookie effacé même OP injoignable, 503 actionnable, échappement HTML) et le câblage par `Auth` (couche INJECTÉE, refus fail-closed, quatre autres modes intacts) — sans réseau NI le paquet `openid-client` (bouchon du contrat OidcClientPort)
  "./test-access.js",            // AUTORISATION (lot 1 auth/ACL) : modèle PARTAGÉ src-shared/Permissions (carte collections → domaines en invariant, catalogue, jokers, presets, permissions d'un lot d'écriture), analyse pure de roles.json (RolesConfig), politique sur fichier fail-closed + rétrocompat + génération (FileRoleProvider, fs réel), gardes d'AccessControl (401 vs 403, ensemble vide refusé, étiquettes, collection → domaine, cache invalidé à chaud), et le VERROU d'exhaustivité « aucune route sans garde » (analyse des sources des routeurs)
  "./test-client-access.js",     // AUTORISATION CÔTÉ CLIENT (lot 2 auth/ACL) : `core/AccessState` (trois états, délégation intégrale au modèle partagé, injection nulle du mode fichier), carte `core/ViewAccess` des vues non-listing (catalogue, anti-coquille), anti-rafale des 403 `core/AccessDenial` (fenêtre par permission, ≠ verrou terminal du 401), et le VERROU « aucune vue du shell sans gating » (analyse des sources de app/main.ts)
  "./test-access-partial.js",    // L'APP SOUS DROITS PARTIELS (correctif auth/ACL 2026-08-17) : niveau d'hydratation `forbidden` (précédence sur lazy, terminal, exclu de hydrateAll/SSE/dérivés mais compté comme non-hydraté), 🚨 INVARIANT ANTI-DESTRUCTION — une collection vide-parce-qu'INTERDITE fait échouer BRUYAMMENT le `PUT /snapshot` et aucune écriture ne part —, assiette de CHARGEMENT ∩ lisibles au point commun `Store.init` (S1) et ZÉRO requête vers l'interdit (compteurs S2, facettes, sections de fiche, résidu M4b, SSE), `core/ViewRestoration` (S3 — courante → bookmarkée → défaut → première visible, hash interdit, plus aucune vue) et le prédicat d'EXPORT COMPLET `hasFullDocumentRead` (masquer plutôt qu'exporter amputé)
