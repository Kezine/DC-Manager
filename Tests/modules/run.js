...
(async () => {
  ...
  console.log("\n• Serveur : ApiRules — périmètre de rechargement (buildChangeset)");
  {
    const { ApiRules } = SERVER("ApiRules.js");
  ...
  console.log("\n• Serveur : ApiRules — création stricte (createConflicts)");
  {
    const { ApiRules } = SERVER("ApiRules.js");
  ...
  console.log("\n• Serveur : ApiRules — cascade résiduelle d'un lot (residualCascade)");
  {
    const { ApiRules } = SERVER("ApiRules.js");
  ...
  console.log("\n• Serveur : Repository / DocumentStore (better-sqlite3 réel)");
  ...
  console.log("\n• RestProtocol : interprétation des réponses (X-Doc-Rev, 409, 400 structuré)");
  {
  ...
  console.log("\n• Store : onPersistError (échec de persistance méta/snapshot NOTIFIÉ)");
  {
    const s = await makeStore();
    let captured = null;
