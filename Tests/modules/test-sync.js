...
module.exports = async () => {
  await section("sync : RenderImpact (carte d'impact 3D)", async () => {
  {
    ...
    ck.eq(COLLECTION_THREE_IMPACT.contacts, "none", "contacts → none (carnet de notifications, hors 3D)");
    ck.eq(COLLECTION_THREE_IMPACT.vms, "none", "vms → none (équipements virtuels, jamais rendus en 3D)");
    ck.eq(COLLECTION_THREE_IMPACT.cableBundles, "geometry", "cableBundles → geometry (tracés de trunk dessinés 2D/3D)");
