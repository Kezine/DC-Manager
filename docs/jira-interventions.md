...
| 3 | 🚨 Recherche par lots sur **`POST /rest/api/3/search/jql`**, corps `{ jql, fields, maxResults }`, pagination par **`nextPageToken`** (Atlassian a remplacé l'ancien `POST …/search` en `startAt`/`total`). ✅ **Constats sur instance réelle** : le chemin **existe** (pas de 404), l'auth y passe, et il **EXIGE une JQL BORNÉE** (400 « Les requêtes JQL non liées ne sont pas autorisées ici » sur une clause sans restriction) — d'où la **sonde du test bornée sur le projet** (`probeJql`, qui fait aussi vérifier `project_key`) ; la clause `id IN (…)` de la synchro **est** une restriction et passe. Restent à constater : la forme de pagination réelle, et le sort d'un id **inexistant** dans `id IN (…)` (omission attendue — fondement de la détection « introuvable » ; un 400 serait à traiter). Si l'instance répondait `404` : mettre `PATH_SEARCH_LEGACY` dans `PATH_SEARCH` et **rien d'autre** — le décodeur et la pagination comprennent **déjà les deux formes** | `PATH_SEARCH` / `PATH_SEARCH_LEGACY`, `JiraAdapter.probeJql`, `JiraParse.page`/`nextCursor` |
