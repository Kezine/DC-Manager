...
| **T1/T2** | règles métier supplémentaires : invariants intra-record (équipement racké ⇒ baie ; port X/Y cohérents ; brosse ⇒ baie) + cross-entité (équipement tient dans la baie ; baie dans les bornes de la salle ; port parent/agrégat même équipement — c'est la SEULE règle que le breakout impose à la validation ; le breakout lui-même (modèle trunk/lanes, gestes éclater/défaire et leurs refus, section dédiée du formulaire) est décrit dans [`breakout.md`](breakout.md)) | ✅ |
