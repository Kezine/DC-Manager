...
/** Enregistrement brut : métadonnées + Blob binaire. `url`/`bytes`/`rev` (optionnels) servent au mode REST,
    où le blob n'est pas chargé en mémoire (le miroir pointe l'URL serveur ; `rev` = révision du BINAIRE côté
    serveur, jeton de cache-busting `?v=` — cf. faceImageUrl). */
export interface ImageRec { id: string; name: string; u_height: number; face: string; with_ears: boolean; description: string; type: string; blob: Blob | null; url?: string; bytes?: number; rev?: number; }
/** Vue miroir (UI) : métadonnées + objectURL synchrone. */
export interface ImageMirror { id: string; name: string; u_height: number; face: string; with_ears: boolean; description: string; type: string; url: string | null; bytes: number; rev?: number; }
...
export class ImageStore {
  ...
  /** Nom de fichier de TÉLÉCHARGEMENT d'une image : nom assaini + extension déduite du MIME
      (aligné sur Schema.IMAGE_MIME_TYPES : PNG / JPEG / WebP). */
  static downloadName(name: string, mime: string): string {
    const ext = mime === "image/png" ? ".png" : mime === "image/webp" ? ".webp" : (mime === "image/jpeg" || mime === "image/jpg") ? ".jpg" : "";
    return Download.safeName(name || "image") + ext;
  }
  ...
  private norm(rec: any): ImageRec {
    ...
    return { id: rec.id, name: rec.name || "", u_height: (face === "autre") ? 1 : Math.max(1, rec.u_height | 0 || 1), face, with_ears: ImageStore.ears(face, rec.with_ears), description: rec.description || "", type: rec.type || (rec.blob && rec.blob.type) || "", blob: rec.blob || null, url: rec.url, bytes: rec.bytes, rev: rec.rev };
  }
  private mirrorPut(rec: ImageRec): void {
    ...
    const face = ImageStore.face(rec.face);
    this.mirror.set(rec.id, { id: rec.id, name: rec.name || "", u_height: (face === "autre") ? 1 : (rec.u_height || 1), face, with_ears: ImageStore.ears(face, rec.with_ears), description: rec.description || "", type: rec.type || "", url, bytes: rec.blob ? rec.blob.size : (rec.bytes || 0), rev: rec.rev });
  }
