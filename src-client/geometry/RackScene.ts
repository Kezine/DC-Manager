...
export interface OccupantElev { u: number; h: number; side: string; depth: string; depth_mm: any; locks_u: boolean; label: string; kind: string; id: string; itemKind?: string; tray_type?: string; tray_u?: number; face_offset_mm?: number; }
  ...
  occupants(rackId: string, opts: { exceptEqId?: string; exceptItemId?: string; exceptBrushId?: string } = {}): Map<string, OccupantInfo> {
    ...
    s.equipmentsOfRack(rackId).forEach((e: any) => {
      if (e.placement_mode === "rack" && e.rack_u && e.id !== opts.exceptEqId) {
        ...
        put(e.rack_u, e.u_height || 1, RackGeometry.mountSides({ depth: e.depth, depth_mm: e.depth_mm, side: e.rack_side, locks_u: e.locks_u }, rack),
