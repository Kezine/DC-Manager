...
export class TrunkRouting {
  ...
  interDcTrunks(m: MultiLayout, portNormal: boolean): Array<{ bundle: any; pts: Vec3[]; linePts: Vec3[]; straight?: Set<number>; stubAt?: Set<number> }> {
    ...
    this.store.all("cableBundles").forEach((bundle: any) => {
      ...
      const cA = this.endpointContainer(bundle, "A"), cB = this.endpointContainer(bundle, "B");
      if (!cA || !cB) return;
      // aligne les extrémités sur le sens de la route (A → conteneur de départ) ; incohérent → non tracé
      // (parité câbles). Comparaison STRUCTURELLE : un étage n'a pas d'id à comparer.
