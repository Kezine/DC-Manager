/* ============================================================================
   Domaine `forms` — ANGLAIS. Calque EXACT de `../fr/forms.ts`. Agrégé par
   `../en.ts`. Termes techniques conservés (breakout, trunk, lane, U, QSFP…). */
export const forms = {
  faceRatio: {
    frontEars: "Faceplate {{u}}U · with ears",
    frontNoEars: "Faceplate {{u}}U · without ears",
    rear: "Faceplate {{u}}U · rear",
  },
  breakout: {
    needPortTypes: "Create port types first (QSFP+ and SFP+).",
    lanes: "Number of lanes: <b>×{{n}}</b>  ({{trunk}} ÷ {{lane}} = {{n}} — standard breakout).",
    nonStandard: "Non-standard combination: {{trunk}} ÷ {{lane}} = {{ratio}}. A valid breakout requires trunk rate = N × lane rate with N ∈ {{spans}}.",
    laneOpt: "×{{n}} lanes",
    lanesField: "Number of lanes",
    lanesManualHint: "Rate not set on these types → manual choice among the standard breakouts.",
    title: "New breakout",
    create: "Create",
    namePlaceholder: "e.g. QSFP1",
    nameField: "Trunk name",
    nameHint: "The lanes will be named « name/1 », « name/2 », …",
    trunkField: "Trunk type (physical connector)",
    trunkHint: "E.g. 400G QSFP-DD — the trunk does not carry a cable itself.",
    laneField: "Lane type",
    laneHint: "Identical for ALL lanes — each carries a 1:1 cable.",
    errName: "Give the trunk a name.",
    errTrunk: "Choose the trunk type.",
    errLane: "Choose the lane type.",
    errCombo: "Non-standard trunk/lane combination: adjust the types (trunk rate = N × lane rate, N ∈ {{spans}}).",
  },
  cap: {
    kept_one: "{{count}} cell kept: a pin is placed on it.",
    kept_other: "{{count}} cells kept: a pin is placed on them.",
    clearAll: "Clear all",
    clearAllTitle: "Remove all holes from this cap (cells bearing a pin are kept)",
  },
  image: {
    badFormat: "Unsupported format (PNG / JPEG / WebP).",
  },
  side: {
    left: "L",
    right: "R",
    bay: "bay",
    marginLeft: "left margin",
    marginRight: "right margin",
    here: "here",
    tooWide: "The equipment (width {{w}} mm) exceeds the column width ({{col}} mm).",
  },
  rack: {
    rearSuffix: " (back)",
    remove: "Remove",
    mount: "Mount an item here",
  },
  ph: {
    equipment: "(equipment)",
  },
  opt: {
    none: "— none —",
    floorNone: "— floor —",
    outOfList: "{{value}} (off list)",
  },
  port: {
    bundleNone: "— bundle —",
    bundleOpt: "{{name}} · {{free}} free",
    strandTx: "Tx",
    strandRx: "Rx",
    strandSimplex: "strand",
    netJoker: "— network (wildcard) —",
    netTitle: "PRIMARY network asserted by this port (network source; empty = wildcard).",
    netRemoved: "{{count}} network(s) removed from the port (wildcard: the port adopts the inferred network).",
    netExtraTitle: "{{count}} additional network(s) (multi editing: coming soon)",
    dirNone: "— direction —",
    ampTitle: "Rating / current ceiling (A).",
    phaseNone: "— phase —",
    phaseTitle: "Phase (three-phase feed spread).",
    category: "Category",
    connector: "Port type",
    sens: "Direction",
    caliber: "Rating",
    phase: "Phase",
    network: "Network",
    strands: "Strands (bundle)",
    pse: "PSE",
    pd: "PD",
    pseLong: "Source (PSE)",
    pdLong: "Powered (PD)",
    poeBudget: "PoE budget",
    poeInject: "PoE injection",
    poeDraw: "PoE draw",
    poeEnabledHint: "Enables injection (PSE) / draw (PD) on this port. The cable's energy bolt only shows if both sides are active.",
    poeClassHint: "PoE classes: 15.4 · 30 · 60 · 90 W",
    poeBudgetHint: "Port capacity (what it can supply). Actual draw comes from the connected PD.",
    poeNormNone: "— standard / budget —",
    poeNormCustom: "Custom…",
    poeConnHint: "PoE runs over Ethernet — data connectors only.",
    noPatchBundle: "No bundle terminates on this patch (attach them from the Bundles tab).",
    bundlesPrefix: "Bundles: ",
    bundleUsage: "{{name}} {{used}}/{{capacity}} strands",
    departs: "Feeds: {{list}}",
    phases: "Phases: {{list}}",
  },
} as const;
