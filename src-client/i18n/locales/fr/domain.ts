...
export const domain = {
  ...
  // CATÉGORIE d'état d'un ticket — énumération FERMÉE (`ISSUE_STATUS_CATEGORIES`), la SEULE partie
  // traduisible de l'état. ⚠ Le libellé `status` du tracker, lui, s'affiche TEL QUEL et n'est JAMAIS
  // traduit (décision D3 : les workflows sont configurables par projet). Résolu au point de rendu par
  // `core/IssueStatus.categoryLabel`. Les trois premières entrées sont VERROUILLÉES par test contre le
  // catalogue de recherche partagé `SEARCH_CATALOGS.issueStatusCategory`.
  issueStatusCategory: {
    todo: "Ouvert",
