/* ============================================================================
   Domaine `lists` — FRANÇAIS. LISTES génériques : chrome de `ListView`
   (recherche, tri, filtres, pagination, actions de ligne) + configurations de
   colonnes de `ListConfigs` (en-têtes, états vides, libellés de filtres et
   d'options, placeholders de cellule). Agrégé par `../fr.ts`. Voir docs/i18n.md.

   `col` = en-têtes de colonnes (réutilisés d'une liste à l'autre) ; `empty` =
   états vides par collection ; `filter`/`opt` = libellés de filtres et d'options ;
   `ph` = placeholders et fragments de cellule (noms de repli, mentions « libre »…). */
export const lists = {
  chrome: {
    searchPlaceholder: "Rechercher…",
    create: "+ Nouveau",
    sort: "Trier",
    sortCreated: "Date de création",
    sortUpdated: "Date de modification",
    dirDesc: "▼ Décroissant",
    dirAsc: "▲ Croissant",
    compact: "Compact",
    compactTitle: "N'afficher que les colonnes essentielles (affichage dense)",
    filter: "Filtrer",
    filterReset: "Réinit. filtres",
    addFilter: "Filtre",
    addFilterTitle: "Ajouter un filtre",
    filterAny: "Tous",
    filterChip: "{{dim}} : {{value}}",
    removeFilter: "Retirer le filtre {{dim}} : {{value}}",
    actions: "Actions",
    empty: "Aucun élément.",
    // État transitoire d'un listing PAGINÉ PAR LE SERVEUR (collections chargées paresseusement, cf.
    // docs/hydratation.md) : tant que la page n'est pas arrivée, on ne dit PAS « aucun élément ».
    loading: "Chargement…",
    count_one: "{{count}} élément · page {{page}}/{{pages}}",
    count_other: "{{count}} éléments · page {{page}}/{{pages}}",
    pageSize: "{{n}}/page",
    rowView: "Détails",
    rowShow: "Afficher",
    rowManage: "Contenu (montage des U)",
    rowEdit: "Modifier",
    rowDelete: "Supprimer",
    rowMore: "Plus d'actions",
    rowLocate: "Localiser en 3D",
    rowDownload: "Télécharger",
    rowClone: "Cloner",
    // Panneau du filtre CIBLE « à recherche » (déclencheur fermé + panneau-portail, refonte
    // 2026-08-03 — ui/FilterBar.openSearchPanel). Le badge « Tous » du déclencheur vient de
    // ui.multiselect.all (parité MultiSelect) ; le nom et le compteur n'ont pas de clé.
    targetPanelLabel: "Filtre {{dim}}",
    targetSelected_one: "Cible",
    targetSelected_other: "Cibles · {{count}}",
    targetRemove: "Retirer {{value}}",
    targetReplace: "Remplacer par…",
    targetAdd: "Ajouter…",
    targetTaken: "déjà pris",
    targetIdle: "Tapez pour chercher une cible.",
    targetNoResult: "Aucune cible ne correspond à « {{q}} ».",
    targetCapped: "{{count}} premiers résultats — affinez la saisie.",
    targetKeyBrowse: "parcourir",
    targetKeyPick: "choisir",
    targetKeyEsc: "Échap",
    targetKeyClose: "fermer",
  },
  col: {
    name: "Nom",
    type: "Type",
    group: "Groupe",
    location: "Emplacement",
    ports: "Ports",
    aggregates: "Agrégats",
    subEquipments: "Sous-équip.",
    ssid: "SSID",
    accessPoint: "Point d'accès",
    connectedSince: "Connecté depuis",
    description: "Description",
    color: "Couleur",
    label: "Label",
    ipNetwork: "Réseau IP",
    cables: "Câbles",
    equipments: "Équipements",
    preview: "Aperçu",
    height: "Hauteur",
    face: "Face",
    usages: "Usages",
    link: "Liaison",
    lengthShort: "Long.",
    status: "Statut",
    networks: "Réseaux",
    strands: "Brins",
    length: "Longueur",
    route: "Route",
    dimensions: "Dimensions",
    room: "Local",
    racks: "Baies",
    address: "Adresse",
    floors: "Étages",
    rooms: "Salles",
    email: "E-mail",
    phone: "Téléphone",
    organization: "Organisation",
    position: "Poste",
    notes: "Notes",
    building: "Bâtiment",
    floor: "Étage",
    size: "Taille",
    depth: "Profondeur",
    faces: "Faces",
    occupied: "Occupé",
    free: "Libre",
    contig: "Contigu",
    kind: "Genre",
    roles: "Rôles",
    family: "Famille",
    connector: "Connecteur",
    speed: "Débit",
    familyPort: "Famille (port)",
    medium: "Média",
    addresses: "Adresses",
    dhcpRanges: "Plages DHCP",
    logicalNetworks: "Réseaux logiques",
    network: "Réseau",
    equipment: "Équipement",
    range: "Plage",
    dhcpServer: "Serveur DHCP",
    designation: "Désignation",
    characteristics: "Caractéristiques",
    assignedTo: "Affecté à",
    storage: "Stockage",
    purchase: "Achat",
    warranty: "Garantie",   // tête de colonne COURTE (≠ `equipment.field.warrantyEnd` « Fin de garantie », trop long)
    ageWarranty: "Âge / garantie",   // colonne COMBINÉE (âge d'achat · état de garantie coloré) — cf. LifecycleFormat
    host: "Hôte",
    tags: "Tags",
    hostname: "Hostname",
    attachedTo: "Attachée à",   // colonne CIBLE d'une pièce jointe (équipement OU sous-équipement)
    file: "Fichier",   // nom d'origine du binaire d'une pièce jointe
  },
  empty: {
    equipments: "Aucun équipement.",
    networks: "Aucun réseau.",
    groups: "Aucun groupe.",
    faceImages: "Aucune image de façade. Ajoutez-en une avec « + Image », ou importez-en depuis l'éditeur de façade d'un équipement.",
    cables: "Aucun câble.",
    cableBundles: "Aucun faisceau.",
    datacenters: "Aucune salle.",
    sites: "Aucun site / bâtiment.",
    contacts: "Aucun contact. Ajoutez-en un avec « + Contact » (destinataire des notifications email / sms).",
    floors: "Aucun étage.",
    racks: "Aucune baie.",
    portTypes: "Aucun type de port.",
    cableTypes: "Aucun type de câble.",
    ipNetworks: "Aucun réseau IP.",
    ipAddresses: "Aucune adresse IP.",
    dhcpRanges: "Aucune plage DHCP.",
    spares: "Aucune pièce de rechange. Ajoutez-en une avec « + Spare » (HDD, SSD, transceiver, autre…).",
    vms: "Aucune VM. Les équipements virtuels sont alimentés par la synchronisation d'un cluster de management (Proxmox).",
    wifiClients: "Aucun client wifi. Les clients sont alimentés par la synchronisation d'un contrôleur wifi (onglet Wifi → « Providers… », mode API).",
    subEquipments: "Aucun sous-équipement. Un sous-équipement se crée depuis la fiche de son équipement maître (pas de bouton « + » ici, v1).",
    applications: "Aucune application. Ajoutez-en une avec « + Application » (hébergée sur un équipement ou une VM, URL optionnelle).",
    attachments: "Aucune pièce jointe. Ajoutez-en une avec « + Pièce jointe » (fichier attaché à un équipement ou un sous-équipement — convention de prêt, bon de commande, garantie…).",
  },
  filter: {
    room: "Salle",
    server: "Serveur",
    // Dimension CIBLE « à recherche » des listings (filtre par entité — cf. views/ListTargets).
    carrier: "Porteur",
    carrierPlaceholder: "Équipement ou VM…",
    hostedOn: "Hébergée sur",   // dimension cible du listing des applications (équipement OU VM — cf. ListTargets.applicationHost)
    attachedTo: "Attachée à",   // dimension cible du listing des pièces jointes (équipement OU sous-équipement — cf. ListTargets.attachmentTarget)
    master: "Équipement (maître)",   // dimension cible du listing des sous-équipements (famille UNIQUE : équipement)
    equipmentPlaceholder: "Rechercher un équipement…",
    targetEquipment: "Équip.",
    targetVm: "VM",
    targetSubEquipment: "Sous-équip.",   // badge de famille « sous-équipement » (dimension cible des pièces jointes)
    targetMissing: "(supprimé)",
  },
  opt: {
    dataFilter: "Data",
    powerFilter: "Alimentation",
    single: "Simple",
    dual: "Double",
    faceOther: "Autre",
    kindDataPill: "data",
    kindPowerPill: "alim.",
    // Options FIXES du filtre-colonne « Âge / garantie » (volet 2 du TODO
    // age-garantie-mise-en-evidence) — ids = les slugs d'état de `LifecycleFormat.warrantyFilterState`.
    warrantyOk: "Sous garantie",
    warrantyWarn: "Expire bientôt",
    warrantyErr: "Expirée",
    warrantyNone: "Sans garantie",
  },
  ph: {
    noName: "(sans nom)",
    group: "(groupe)",
    noLabel: "(sans label)",
    network: "(réseau)",
    cable: "(câble)",
    bundle: "(faisceau)",
    room: "(salle)",
    site: "(site)",
    contact: "(contact)",
    rack: "(baie)",
    equipment: "(équip.)",
    server: "(serveur)",
    vm: "(VM)",
    spare: "(spare)",
    equipDeleted: "(équip. supprimé)",
    free: "libre",
    logical: "logique",
    direct: "directe",
    freeAddr: "— libre —",
    notDesignated: "— non désigné —",
    orphan: "orpheline",
    // Pendant WIFI de « orpheline » : MÊME mécanique (absent du dernier inventaire), autre SENS —
    // un client wifi qui part est ordinaire, pas un incident (cf. core/WifiStatus, décision D2).
    // ⚠ VERROUILLÉ par test contre le catalogue partagé `SEARCH_CATALOGS.wifiDisconnected`.
    disconnected: "déconnecté",
    // Pendant TICKET de « orpheline »/« déconnecté » : MÊME mécanique (non résolu à la dernière
    // passe), TROISIÈME sens — un ticket qu'on ne retrouve plus signale une suppression, un projet
    // archivé ou une permission perdue. AFFICHÉ par le bloc « Ticket » d'une intervention répliquée,
    // à la place de la sentinelle brute écrite par le pont (cf. core/TrackerStatus).
    notFound: "introuvable",
    wifiClient: "(client wifi)",
    // Hôte d'une application dont la FK ne résout plus (équipement/VM supprimé hors cascade, import…) :
    // l'orphelin se VOIT — il ne se déguise jamais en « aucun hôte » (patron des cibles d'intervention).
    hostMissing: "(introuvable)",
    floorLabel: "Étage {{n}}",
    addrCount: "{{count}} adr.",
    points_one: "{{count}} point",
    points_other: "{{count}} points",
  },
} as const;
