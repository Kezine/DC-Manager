...
export interface CtxItem { label: string; danger?: boolean; disabled?: boolean; title?: string; action: () => void; }
  ...
  static show(clientX: number, clientY: number, sections: CtxSection[]): void {
    ...
    sections.forEach((sec, si) => {
      ...
      sec.items.forEach((it) => {
        ...
        if (it.disabled) { b.disabled = true; b.onclick = (e) => e.stopPropagation(); }   // grisé (CSS button:disabled) — action neutralisée
        else b.onclick = (e) => { e.stopPropagation(); ContextMenu.hide(); it.action(); };
        m.appendChild(b);
      });
    });
