...
export class InterventionsAdminView {
  ...
  /** Icône de FAMILLE d'une cible liable — repère visuel de la liste des objets liés.
      ...
      déjà la distinction. `application` a en revanche son PROPRE repère (elle n'est pas du matériel).
      Le défaut EQUIPMENT reste le repli des slugs inconnus. */
  private static familyIcon(kind: string): string {
    return kind === "vm" ? Icons.VM : kind === "spare" ? Icons.SPARE : kind === "application" ? Icons.APPLICATION : Icons.EQUIPMENT;
  }
