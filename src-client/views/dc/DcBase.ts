...
export abstract class DcBase {
  ...
  protected trunks: TrunkRouting;                // service de routage des FAISCEAUX (uplinks de patch — même mécanique)
  ...
  // FLUSH de la persistance à la fermeture/masquage de page : `persistView` est débouncé (300 ms) ; un rechargement
  // (F5) ou une fermeture n'attend PAS les timers en attente → la dernière bascule (DC actif, vue, caméra) serait
  // PERDUE. On écrit alors immédiatement (synchrone) sur `pagehide` + `visibilitychange`(masqué).
  ...
  constructor(store: Store, mount: HTMLElement, host: DatacenterHost = {}) {
    this.store = store; this.host = host; this.stage = mount; this.scene = new RackScene(store); this.resolver = new Resolver3D(store); this.floor = new FloorLayout(store); this.routing = new CableRouting(store, this.resolver, this.floor); this.trunks = new TrunkRouting(store, this.resolver, this.routing);
    ...
    // flush synchrone de l'état de vue avant que la page ne parte (refresh/fermeture) ou ne passe en arrière-plan.
    this._onPageHide = () => this.flushViewState();
    window.addEventListener("pagehide", this._onPageHide);
    this._onVisChange = () => { if (document.visibilityState === "hidden") this.flushViewState(); };
  ...
  dispose(): void {
    ...
    if (this._onVisChange) { document.removeEventListener("visibilitychange", this._onVisChange); this._onVisChange = null; }
    this.flushViewState();   // dernière écriture synchrone avant destruction (idempotent, gardé par _restoredKey)
  ...
  show(): void {
    ...
    // GARDE « document chargé » : au boot REST, le document arrive de façon ASYNCHRONE (bootstrap) APRÈS que le shell
    // ait restauré l'onglet bookmarké → ce 1er `show()` tourne sur un store VIDE. Restaurer là manquerait le DC sauvé
    // (aucune salle → `has()` échoue) ET verrouillerait `_restoredKey` sur une clé prématurée. On NE restaure donc (et
    // ne verrouille) QUE si des salles existent ; `documentOpened` rappelle `show()` une fois le doc chargé (switchView
    // ré-exécute onShow) → restauration correcte. Tant que `_restoredKey` reste null, `writeViewState` n'écrit RIEN →
    // aucun écrasement de l'état sauvé pendant cette fenêtre pré-chargement.
    const key = this.viewStateKey();
    if (key !== this._restoredKey && this.dcs().length > 0) { this.restoreView(); this._restoredKey = key; }
  ...
  protected webglCtx(): any {
    ...
    const pushTrunk = (bundle: any, linePts: any[], straight?: Set<number>, stubAt?: Set<number>) => {
      if (!shown(bundle)) return;
      extraCables.push({ id: bundle.id, color: null, line: linePts.map((p) => ({ x: p.x, y: p.y, z: p.z })), straight: straight ? [...straight] : [], stubAt: stubAt ? [...stubAt] : [], power: false, kind: "trunk" });
    };
  ...
  protected webglTipHtml(desc: any): string | null {
    ...
    switch (desc.type) {
      ...
      case "keepblankReserve": { const it = s.get("rackItems", desc.id); return it ? this.itemTipHtml(it) : null; }
      ...
      case "trunk": { const b = s.get("cableBundles", desc.id); return b ? this.bundleTipHtml(b) : null; }
  ...
  protected webglContextMenu(desc: any, x: number, y: number): void {
    ...
    switch (desc.type) {
      ...
      case "keepblankReserve": { const it = s.get("rackItems", desc.id); sections = it ? this.itemCtx(it) : null; break; }   // clic droit sur la bande KeepBlank → actions du pseudo-élément
      ...
      case "trunk": { const b = s.get("cableBundles", desc.id); sections = b ? this.bundleCtx(b) : null; break; }
      ...
      case "floorplane": sections = this.floorPlane3DCtx(desc.loc, desc.floor); break;   // dalle d'étage nue (multi-salles) : éditer plan / + salle / vue 2D
    }
  ...
  protected persistView(): void {
    ...
    this._pvTO = setTimeout(() => this.writeViewState(), 300);
  }

  /** Écrit l'état de vue IMMÉDIATEMENT (annule le debounce en attente). Appelé sur `pagehide`/`visibilitychange`
      et au `dispose` : garantit que le DC actif, la vue et la caméra survivent à un rechargement (F5) qui n'attend
      pas les timers. Idempotent. */
  protected flushViewState(): void {
    clearTimeout(this._pvTO);
    this.writeViewState();
  }

  /** Sérialise l'état de vue courant dans localStorage. GARDE `_restoredKey` : tant que la vue Datacenter n'a pas
      été activée (donc restaurée) cette session, on n'écrit RIEN — sinon un flush de fermeture écraserait l'état
      sauvé (DC actif, caméra…) par les valeurs par défaut d'une vue jamais ouverte. */
  ...
  protected abstract bundleTipHtml(bundle: any): string;
  ...
  protected abstract floorPlane3DCtx(loc: string, floor: string): CtxSection[];
  ...
  protected abstract bundleCtx(bundle: any): CtxSection[];
