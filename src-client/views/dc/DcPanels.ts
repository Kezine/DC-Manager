...
export abstract class DcPanels extends DcViews2D {
  ...
  protected roomPickerCard(dc: any): HTMLElement {
    const box = document.createElement("div"); box.className = "dc-card";
    const t = document.createElement("div"); t.className = "dc-card-title"; t.textContent = "Salle"; box.appendChild(t);
    ...
    locs.forEach((loc) => {
      ...
      inLoc.forEach((d: any) => {
        ...
        const tog = FormControls.toggle((d.name || "(salle)") + (isCur ? "  ◀ active" : ""), isCur, () => {
          if (isCur) return;
  ...
  protected cableCard(dc: any): HTMLElement {
    ...
    if (trunkIds().length) {
      const row = document.createElement("div"); row.className = "dc-layer-row";
      const ttx = document.createElement("span"); ttx.className = "grow"; ttx.textContent = "Faisceaux (trunks) · " + trunkIds().length;
      row.append(ttx); eyePair(row, trunkIds, "les faisceaux"); box.appendChild(row);
    }
  ...
  protected view3dOptionsCard(): HTMLElement {
    ...
    tgi(ALL, I.perp, "Sortie ⊥ des ports (20 mm) : les câbles quittent la face perpendiculairement sur 20 mm (affecte aussi le tracé 2D)", () => this.cablePortNormal, (v) => { this.cablePortNormal = v; redraw(); });
    section("Waypoints");
    ...
    tgi(D3, I.realSize, "Taille RÉELLE des marqueurs : waypoints 5 cm, pastilles de câble ⌀ 1 cm (à 100 % du slider) — suivent le zoom comme la géométrie. Désactivé : taille écran constante quel que soit le zoom.", () => this.markerRealSize, (v) => { this.markerRealSize = v; if (this.useWebGL && this._three) { this._three.setMarkerRealSize(v); this.persistView(); } else redraw(); });
