...
export abstract class DcViews2D extends DcScene3D {
  ...
  renderTop(dc: any): void {
    ...
    this.drawTrunks2D(gRoot, dc);   // faisceaux (trunks) — même filtre, trait plus épais, clic → form faisceau
  ...
  renderFloor(ft: { location: string; floor: string }): void {
    ...
    // faisceaux inter-DC de l'étage (uplink patch → exits/OOB → uplink patch) — même filtre que les câbles
    {
  ...
  protected floorDrag(e: MouseEvent, opts: {
    entityId?: string; delegatePos?: boolean; locked?: boolean;
    ...
    if (opts.locked) {   // positionnement VERROUILLÉ : sélection seule, aucun déplacement (le formulaire reste l'échappatoire)
      e.preventDefault(); e.stopPropagation();
      opts.preselect?.(e.currentTarget as SVGElement); opts.onClick?.();
      return;
    }
  ...
  protected onFloorEquipPointerDown(e: MouseEvent, eq: any, cfg: any): void {
    ...
    this.floorDrag(e, {
      entityId: eq.id, delegatePos: true, locked: PlacementLock.isLocked(eq), rotate: o,
  ...
  protected onFloorOobPointerDown(e: MouseEvent, wp: any, cfg: any): void {
    ...
    const clamp = (p: { x: number; y: number }) => ({ x: Math.min(Math.max(p.x, 0), W), y: Math.min(Math.max(p.y, 0), D) });
    this.floorDrag(e, {
      start: FloorLayout.oobFloorPos(wp, cfg), clamp, hideTipOnMove: true, locked: PlacementLock.isLocked(wp),
      preselect: (grp) => {
  ...
  protected doorSwingNode(r: any): SVGElement | null {
    ...
    (["front", "rear"] as const).forEach((face) => {
      ...
      any = true;
      // un secteur + un gond PAR VANTAIL (1 simple · 2 double battant) — géométrie partagée avec la 3D.
      RackDoorGeometry.swingSectors(w, d, face === "rear", dr).forEach((s) => {
        const pts = RackDoorGeometry.sectorPointsOf(s, 16);
  ...
  protected waypointNode2D(wp: any, dc: any): SVGElement {
    ...
    const startDrag = (ev: MouseEvent, which: string) => {
      ...
      this.renderSide(dc);
      if (PlacementLock.isLocked(wp)) return;   // positionnement VERROUILLÉ : sélection seule, aucun déplacement
  ...
  protected drawTrunks2D(gRoot: SVGElement, dc: any): void {
    const rDot = DC_DOT_PX * this.markerScale / (this.scale || 1);
    this.resolvedTrunks(dc.id).forEach((rt) => { if (this.trunkShown(rt)) this.drawTrunk2D(gRoot, rt, rDot); });
    this.outgoingTrunkStubs(dc.id).forEach((st) => { if (this.trunkShown(st)) this.drawTrunk2D(gRoot, st, rDot); });
  }

  /** Trace UN faisceau : même spline/amorces que les câbles, trait plus ÉPAIS et couleur NEUTRE (un trunk n'a
      pas UN réseau) ; clic → formulaire FAISCEAU ; survol/menu = mêmes mécanismes que les câbles. */
  protected drawTrunk2D(gRoot: SVGElement, rt: { bundle: any; pts: Vec3[]; linePts: Vec3[]; straight?: Set<number>; stubAt?: Set<number> }, rDot: number): void {
    const line2 = rt.linePts.map((p) => ({ h: p.x, v: p.y }));   // tracé (avec amorces)
    ...
    g.append(line, hit); gRoot.appendChild(g);
    [ends[0], ends[ends.length - 1]].forEach((p) => gRoot.appendChild(Dom.svg("circle", { class: "dc-cable-end dc-trunk-end", cx: p.h, cy: p.v, r: rDot })));
  }
