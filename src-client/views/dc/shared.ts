...
export interface DatacenterHost {
  ...
  openRackContentForm?(id: string): void;     // éditeur de CONTENU d'une baie (montage des U) — modale dédiée interactive
  ...
  openCableBundleForm?(id: string | null): void;      // édition d'un FAISCEAU (trunk) — clic sur son tracé 2D/3D
  openWaypointForm?(id: string | null, opts?: any): void;
  openRackItemForm?(id: string): void;        // édition d'un pseudo-élément de baie (tray/blank/keepblank)
  assignTraySlot?(trayItemId: string, onDone?: () => void): void;   // poser un équipement libre sur une étagère
  removeRackItem?(id: string, onDone?: () => void): void;           // retirer un pseudo-élément (ex. libérer une réservation KeepBlank)
