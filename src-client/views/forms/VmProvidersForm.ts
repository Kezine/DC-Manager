...
export class VmProvidersForm {
  ...
  static open(host: FormHost, client: VmSyncClient, onChanged: () => void): void {
    ...
    form.host.openModal({
      title: I18n.t("vm.providers.title"),
      subtitle: I18n.t("vm.providers.subtitle"),
  ...
  private async loadList(): Promise<void> {
    this.message(I18n.t("vm.providers.loading"));
    try {
      ...
      this.message(I18n.t("vm.providers.loadError", { detail: VmProvidersForm.errText(e) }), true);
    }
  }
  ...
  private renderList(providers: VmProviderSummary[]): void {
    ...
    intro.textContent = I18n.t("vm.providers.intro");
    ...
    if (!providers.length) {
      ...
      empty.textContent = I18n.t("vm.providers.empty");
      ...
      const rows = providers.map((p) => [
        ...
        p.interval_sec > 0 ? (p.interval_sec + " s") : I18n.t("vm.providers.intervalManual"),
        p.timeout_sec + " s",
        `<button class="btn btn-ghost btn-sm" data-edit="${Html.escape(p.id)}">${Html.escape(I18n.t("lists.chrome.rowEdit"))}</button>`,
      ]);
      const tw = this.table([I18n.t("vm.providers.colProvider"), I18n.t("vm.providers.colType"), I18n.t("vm.providers.colNodes"), I18n.t("vm.providers.colInterval"), I18n.t("vm.providers.colTimeout"), ""], rows);
    ...
    add.textContent = I18n.t("vm.providers.add"); add.style.marginTop = "12px";
  ...
  private renderForm(existing: VmProviderSummary | null): void {
    ...
    back.textContent = I18n.t("vm.providers.back"); back.onclick = () => void this.loadList();
    this.panel.appendChild(back);

    const heading = document.createElement("div"); heading.className = "section-divider";
    heading.textContent = editing ? I18n.t("vm.providers.headingEdit", { id: existing!.id }) : I18n.t("vm.providers.headingNew");
    this.panel.appendChild(heading);

    // -- id (immuable en édition — c'est la clé de réconciliation des VMs). --
    const idInput = FormControls.text(existing ? existing.id : "", I18n.t("vm.providers.idPlaceholder"));
    if (editing) { idInput.readOnly = true; idInput.style.opacity = "0.7"; }
    this.panel.appendChild(FormControls.fieldRow(I18n.t("vm.providers.idField"), idInput,
      editing ? I18n.t("vm.providers.idHintEdit") : I18n.t("vm.providers.idHintNew")));
    ...
    this.panel.appendChild(FormControls.fieldRow(I18n.t("vm.providers.typeField"), kindSel, I18n.t("vm.providers.typeHint")));
    ...
    const renderPool = (): void => {
      ...
      pool.forEach((row, i) => {
        ...
        const fpI = FormControls.text(row.fingerprint, I18n.t("vm.providers.fpPlaceholder"));
        fpI.oninput = () => { row.fingerprint = fpI.value; };

        // Réordonnancement (monter/descendre) + suppression — l'ordre pilote la bascule de nœud.
        const up = VmProvidersForm.iconBtn("↑", I18n.t("vm.providers.moveUp"), i === 0);
        up.onclick = () => { if (i > 0) { [pool[i - 1], pool[i]] = [pool[i], pool[i - 1]]; renderPool(); } };
        const down = VmProvidersForm.iconBtn("↓", I18n.t("vm.providers.moveDown"), i === pool.length - 1);
        down.onclick = () => { if (i < pool.length - 1) { [pool[i + 1], pool[i]] = [pool[i], pool[i + 1]]; renderPool(); } };
        const del = VmProvidersForm.iconBtn(Icons.CLOSE, I18n.t("vm.providers.removeNode"), pool.length <= 1);
        ...
        const urlField = FormControls.fieldRow(I18n.t("vm.providers.nodeUrlLabel", { n: i + 1 }), urlI); urlField.style.flex = "1 1 260px";
        const fpField = FormControls.fieldRow(I18n.t("vm.providers.tlsField"), fpI); fpField.style.flex = "1 1 220px";
        line.append(urlField, fpField, ctrls);
        poolWrap.appendChild(line);
      });
    };
    renderPool();
    this.panel.appendChild(VmProvidersForm.sectionHint(I18n.t("vm.providers.poolTitle"), I18n.t("vm.providers.poolHint")));
    ...
    addNode.textContent = I18n.t("vm.providers.addNode"); addNode.style.marginTop = "4px";
    ...
    this.panel.appendChild(FormControls.fieldRow(I18n.t("vm.providers.caField"), caInput,
      I18n.t("vm.providers.caHint")));
    ...
    this.panel.appendChild(FormControls.fieldRow(I18n.t("vm.providers.mgmtField"), mgmtInput,
      I18n.t("vm.providers.mgmtHint")));

    // -- Jeton : champ password JAMAIS pré-rempli. En édition, vide = conserver le jeton stocké. --
    const tokenInput = FormControls.text("", editing ? I18n.t("vm.providers.tokenPlaceholderEdit") : I18n.t("vm.providers.tokenPlaceholderNew"));
    tokenInput.type = "password"; tokenInput.autocomplete = "new-password"; // empêche l'autofill du navigateur
    this.panel.appendChild(FormControls.fieldRow(I18n.t("vm.providers.tokenField"), tokenInput,
      editing ? I18n.t("vm.providers.tokenHintEdit") : I18n.t("vm.providers.tokenHintNew")));

    // -- include_lxc / interval_sec / timeout_sec. --
    const lxcToggle = FormControls.toggle(I18n.t("vm.providers.lxcToggle"), existing ? existing.include_lxc : true, () => { /* état lu à l'envoi */ });
    const lxcField = document.createElement("div"); lxcField.className = "form-field";
    const lxcLabel = document.createElement("label"); lxcLabel.textContent = I18n.t("vm.providers.lxcLabel"); lxcField.append(lxcLabel, lxcToggle);
    ...
    this.panel.appendChild(FormControls.fieldRow(I18n.t("vm.providers.intervalField"), intervalInput, I18n.t("vm.providers.intervalHint")));
    const timeoutInput = FormControls.number(existing ? existing.timeout_sec : 15, { min: 1, step: 1, placeholder: "15" });
    this.panel.appendChild(FormControls.fieldRow(I18n.t("vm.providers.timeoutField"), timeoutInput, I18n.t("vm.providers.timeoutHint")));
    ...
    testBtn.textContent = I18n.t("vm.providers.test");
    testBtn.onclick = async () => {
      errBox.style.display = "none";
      testBox.innerHTML = ""; testBox.appendChild(VmProvidersForm.hint(I18n.t("vm.providers.testing")));
    ...
    saveBtn.onclick = async () => {
      ...
      if (!editing && input.id === "") { showError(new VmSyncError(I18n.t("vm.providers.idRequired"), 0, null)); return; }
      saveBtn.disabled = true;
      try {
        await this.client.saveProvider(input.id, input);
        Notify.toast(editing ? I18n.t("vm.providers.savedUpdated") : I18n.t("vm.providers.savedCreated"), "ok");
    ...
    if (editing) {
      ...
      delBtn.onclick = async () => {
        const ok = await Dialog.confirm({
          title: I18n.t("vm.providers.deleteTitle"),
          message: I18n.t("vm.providers.deleteMessage", { id: existing!.id }),
        ...
        try {
          ...
          Notify.toast(I18n.t("vm.providers.deleted"), "ok");
          this.onChanged();
          await this.loadList();
        } catch (e) {
          showError(e);
  ...
  private renderTestResult(box: HTMLElement, info: VmProviderInfo): void {
    ...
    pills.innerHTML = [
      VmProvidersForm.pill(info.ok ? I18n.t("vm.providers.testConnOk") : I18n.t("vm.providers.testConnFail"), info.ok ? "ok" : "err"),
      info.version ? VmProvidersForm.pill(I18n.t("vm.common.pveVersion", { version: info.version }), "neutral") : VmProvidersForm.pill(I18n.t("vm.common.versionUnknown"), "dim"),
      VmProvidersForm.pill(info.supported ? I18n.t("vm.common.rangeSupported") : I18n.t("vm.common.rangeOut"), info.supported ? "ok" : "warn"),
    ].join(" ");
  ...
  private renderDisabled(err: VmSyncError): void {
    ...
    title.textContent = err.message || I18n.t("vm.providers.disabledTitle");
    box.appendChild(title);
    const detail = document.createElement("div"); detail.className = "form-hint"; detail.style.whiteSpace = "pre-line";
    detail.textContent = err.detail || I18n.t("vm.providers.disabledDetail");
