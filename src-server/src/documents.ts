...
export class DocumentStore {
  ...
  constructor(private readonly dir: string, private readonly Database: SqliteCtor, private readonly log: Logger = new Logger("error")) {
    ...
    this.registry.pragma("synchronous = NORMAL");   // sûr en WAL, assumé (parité Repository.open)
  ...
  /** ARRÊT PROPRE : ferme tous les dépôts ouverts puis le registre (chaque close = optimize + checkpoint du -wal,
      cf. Repository.close). Appelé par le handler SIGINT/SIGTERM du bootstrap — sans lui, l'OS ferme les fd mais
      laisse des -wal non checkpointés (recouvrés à la réouverture, jamais corrompus, juste volumineux). */
  closeAll(): void {
    for (const [id, repo] of this.repos) { try { repo.close(); } catch { /* déjà fermé */ } this.log.debug("dépôt fermé", id); }
    this.repos.clear();
    try { this.registry.pragma("optimize"); this.registry.pragma("wal_checkpoint(TRUNCATE)"); } catch { /* driver réduit */ }
    try { this.registry.close(); } catch { /* déjà fermé */ }
    this.log.info("arrêt propre : dépôts et registre fermés");
  }
}
