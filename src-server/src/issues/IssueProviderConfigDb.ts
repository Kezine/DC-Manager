...
/* =============================================================================
   ...
   AES-256-GCM, MÊME clé `DCMANAGER_SECRETS_KEY` que les modules vm/, wifi/ et
   notify/, ce qui est VOULU : une seule clé d'infrastructure à gérer). Un backup du
   fichier n'expose aucun jeton.
   ...
   (pool 1-N ordonné) ; un tracker n'a qu'UNE instance, la config tient dans une
   ...
   JAMAIS de throw global), écriture en transaction, `close()` = checkpoint
   TRUNCATE + optimize.
...
/** Placeholder de jeton NON VIDE injecté pour satisfaire la règle « token requis » de la
    ...
    stocké ni lu : seuls les AUTRES champs validés sont retenus, le `token_enc` existant est
    conservé tel quel. Un caractère de contrôle en tête le rend non collisionnable avec un vrai
    jeton et non affichable.
    ⚠ Écrit en SÉQUENCE D'ÉCHAPPEMENT (`\u0000`) et jamais en caractère brut : un NUL tapé en clair
    ressort tel quel dans le JS compilé et fait passer le fichier pour binaire aux yeux de la
    plupart des outils (piège déjà rencontré dans ce dépôt). */
...
export interface IssueProviderListItem {
  ...
  kind: string;
  url: string;
  /** Compte de service — moitié PUBLIQUE de l'identification, donc RELUE et réaffichée à l'édition
      (c'est toute la différence avec le jeton, qui ne ressort jamais). */
  account: string;
  interval_sec: number;
  timeout_sec: number;
...
export class IssueProviderConfigDb implements IssueProviderConfigSource {
  ...
  constructor(
    ...
    // Réglages de parité avec DocumentStore/Repository (WAL + anti-SQLITE_BUSY). `foreign_keys` est
    // posé par cohérence bien qu'AUCUNE FK n'existe ici (table unique) : le jour où une table fille
    // apparaîtrait, l'oubli du PRAGMA serait un bug silencieux (OFF par défaut).
  ...
  /** Schéma : UNE table (cf. en-tête). `token_enc` = jeton CHIFFRÉ (jamais en clair) ; `account` =
      identifiant PUBLIC du compte de service ; `options` = JSON des réglages propres à la marque.
  ...
  /** Résumés SANS jeton des providers d'un document (id/kind/interval_sec) — matière du STATUT et
  ...
  /** Balaye les providers d'un document en VÉRIFIANT la déchiffrabilité de chaque jeton, MUTUALISE
      la gestion d'erreur des deux surfaces de lecture et projette chaque ligne DÉCHIFFRABLE via
      `project` (qui reçoit le jeton en clair — libre de le consommer ou de l'ignorer). Un jeton
      indéchiffrable → ligne EXCLUE + erreur mémorisée (jamais de throw global). Le message de
  ...
  /** Écrit UN provider (upsert par PK) en UNE transaction. `createdBy` = id canonique de l'auteur
