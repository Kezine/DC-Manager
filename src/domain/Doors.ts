...
export interface DoorSpec {
  wall: DoorWall; offset: number; width_mm: number; height_mm: number; frame_mm: number; hinge: DoorHinge; opening: DoorOpening;
}
...
export class Doors {
  ...
  static defaults(wall: DoorWall, wallLen: number): DoorSpec {
    return { wall, offset: Math.round(Math.max(0, wallLen) / 2), width_mm: DOOR_DEFAULT_WIDTH_MM, height_mm: DOOR_DEFAULT_HEIGHT_MM, frame_mm: DOOR_DEFAULT_FRAME_MM, hinge: "left", opening: "interior" };
  }
}
