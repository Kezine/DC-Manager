...
export class CableRouting {
  ...
  /** Couleur d'un câble = couleur de son réseau principal (null sinon). */
  cableColor(c: any): string | null { const n: any = c && c.network_id ? this.store.get("networks", c.network_id) : null; return (n && n.color) ? n.color : null; }
  ...
  outgoingCableStubs(dcId: string, portNormal: boolean): Array<{ cable: any; portId: string; port: Vec3; portRackId: string | null; pts: Vec3[]; linePts: Vec3[]; straight?: Set<number>; stubAt?: Set<number> }> {
    ...
    this.store.all("cables").forEach((c: any) => {
      ...
      if (!inRoom.length || Waypoint.typeOf(inRoom[portAtStart ? inRoom.length - 1 : 0]) !== "exit") return;
      ...
      const sp = !viaW.length ? { pts: [portRes as Vec3], linePts: [portRes as Vec3], straight: new Set<number>(), stubAt: new Set<number>() }
        : portAtStart ? this.cableLine(portRes, viaW[viaW.length - 1].p, viaW.slice(0, -1), portNormal)
        : this.cableLine(viaW[0].p, portRes, viaW.slice(1), portNormal);
  ...
  interDcRoutes(m: MultiLayout, portNormal: boolean): Array<{ cable: any; a: any; b: any; pts: Vec3[]; linePts: Vec3[]; straight?: Set<number>; stubAt?: Set<number> }> {
    ...
    this.store.all("cables").forEach((c: any) => {
      ...
      const sp = this.cableLine(aw, bw, via, portNormal);
